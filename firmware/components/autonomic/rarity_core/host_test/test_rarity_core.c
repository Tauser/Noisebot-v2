...
static void test_sneeze_rate_limited_and_counted(void)
{
    ...
    CHECK(nb_rarity_core_trigger_sneeze(&r, 0u));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_SNEEZE) == 1u);

    CHECK(!nb_rarity_core_trigger_sneeze(&r, 1000u));
    CHECK(!nb_rarity_core_trigger_sneeze(&r, NB_RARITY_SNEEZE_MIN_INTERVAL_MS - 1u));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_SNEEZE) == 1u); /* tentativas falhas não incrementam */

    CHECK(nb_rarity_core_trigger_sneeze(&r, NB_RARITY_SNEEZE_MIN_INTERVAL_MS));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_SNEEZE) == 2u);
}
...
static void test_dream_requires_sleeping(void)
{
    nb_rarity_state_t r;
    nb_rarity_core_init(&r);
    CHECK(!nb_rarity_core_trigger_dream(&r, false));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_DREAM) == 0u);
}
...
static void test_dream_at_most_once_per_sleep_session(void)
{
    ...
    CHECK(nb_rarity_core_trigger_dream(&r, true));
    for (int i = 0; i < 50; ++i) {
        CHECK(!nb_rarity_core_trigger_dream(&r, true));
    }
    CHECK(nb_rarity_core_count(&r, NB_RARITY_DREAM) == 1u);
}
...
static void test_dream_rearms_after_leaving_and_reentering_sleep(void)
{
    ...
    CHECK(!nb_rarity_core_trigger_dream(&r, false)); /* saiu de SLEEPING */
    CHECK(!nb_rarity_core_trigger_dream(&r, false));

    CHECK(nb_rarity_core_trigger_dream(&r, true)); /* nova sessão -- elegível de novo */
    CHECK(nb_rarity_core_count(&r, NB_RARITY_DREAM) == 2u);
}
...
static void test_stargaze_same_session_rules_independent_of_dream(void)
{
    ...
    CHECK(!nb_rarity_core_trigger_stargaze(&r, false));
    CHECK(nb_rarity_core_trigger_stargaze(&r, true));
    CHECK(!nb_rarity_core_trigger_stargaze(&r, true));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_STARGAZE) == 1u);

    /* DREAM continua com seu próprio estado, intocado. */
    CHECK(nb_rarity_core_count(&r, NB_RARITY_DREAM) == 0u);
    CHECK(nb_rarity_core_trigger_dream(&r, true));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_DREAM) == 1u);
    CHECK(nb_rarity_core_count(&r, NB_RARITY_STARGAZE) == 1u); /* stargaze não mudou */

    CHECK(!nb_rarity_core_trigger_stargaze(&r, false));
    CHECK(nb_rarity_core_trigger_stargaze(&r, true));
    CHECK(nb_rarity_core_count(&r, NB_RARITY_STARGAZE) == 2u);
}

static void test_null_is_safe(void)
{
    nb_rarity_core_init(NULL);
    CHECK(!nb_rarity_core_trigger_sneeze(NULL, 0u));
    CHECK(!nb_rarity_core_trigger_dream(NULL, true));
    CHECK(!nb_rarity_core_trigger_stargaze(NULL, true));
