...
/*
 * Núcleo C17 puro do idle "3 motores" (S3.7 completo, RFC-VIDA-V2.md §7):
 * atenção (nb_attention.h, FIXATE⇄SACCADE), postura (nb_posture.h,
 * deriva de baseline) e energia (nb_energy.h, sonolência contínua), mais
 * respiração (nb_breath.h) e o processo de blink independente (Poisson)
 * + gestos nomeados (CHECK_IN/SLOW_BLINK/SIGH), todos disputando o mesmo
 * slot de exclusividade (`active_motif`).
 *
 ...
 * esp_random() pra ficar host-testável byte a byte. A casca semeia com
 * entropia real (esp_random()) e liga a saída ao renderer (S2.2).
 *
 * Histórico: o catálogo de motifs de S2.4 (SOFT_DRIFT, CURIOUS_TILT,
 * HEAD_TILT_HOLD, LOOK_DOWN_BLINK, LINE_BLINK, SIDE_PEEK, VERTICAL_SCAN,
 * CROSS_SCAN) e a flag de compilação `NB_IDLE_V2_SPIKE` que alternava
 * entre os dois modelos foram aposentados na S3.7 completo, item 8 --
 * o modelo de 3 motores é o único caminho agora (docs/ROADMAP.md).
 */
...
typedef enum {
    ...
    /* S3.7 completo, item 4: gestos nomeados (RFC-VIDA-V2.md §7) -- mesmo
     * slot exclusivo do blink (nunca dois ao mesmo tempo). Fisiologia/
     * automanutenção (Regra da Causa, RFC §2), não "atenção" fingida. */
...
/* Saída por tick -- o que o renderer (S2.2) soma ao estado paramétrico
 * da face. */
typedef struct {
    float gaze_x; /* [-1, 1] */
    float gaze_y; /* [-1, 1] */
    ...
    float open_r;
    float width_l; /* multiplicador de largura, 1 = normal */
    float width_r;
    float tilt; /* [-1, 1]: assimetria vertical (roll) */
    nb_idle_motif_t active_motif;

    /* S3.7 completo, item 3 (acoplamentos): fator de respiração deste
     * tick (mesmo valor aplicado a open_l/open_r) -- exposto pra casca
     * sincronizar o LED idle em fase com a respiração (RFC §7,
     * "respiração ... em fase com o LED idle"). */
    float breath_scale;
} nb_idle_output_t;

/* Métricas acumuladas -- usadas pelo host-test e pela casca pra
 * log/telemetria. */
typedef struct {
    uint32_t blink_bar_count;
    uint32_t double_blink_count;
    uint32_t last_event_at_ms; /* último instante em que algo mudou (blink ou gesto) */
...
typedef struct {
    ...
    /* Sub-fase para motifs com sequência interna (DOUBLE_BLINK): 0 =
     * fase única/inicial. */
    uint8_t motif_phase;
    ...
    /* Gaze resolvido pelo motor de atenção (nb_attention.h) a cada tick. */
    float drift_x;
    float drift_y;
     ...
     * (nunca fica sonolento por padrão). */
     ...
     * somado ao tilt em compute_output(). */
     ...
     * slot de exclusividade (active_motif) que o blink. */
...
/* Modo quiet (frequências ÷2) e atenção (reservado -- ainda não modula o
 * motor de atenção diferente entre IDLE/ATTENTIVE nesta fase). */
