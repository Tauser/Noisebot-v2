...
ESP32-S3** (Freenove CAM N16R8) como corpo + sistema nervoso autonômico, e um
server Python local como mente. Não existe segundo MCU, não existe protocolo
inter-MCU. A fonte de verdade da execução é `docs/ROADMAP.md` (fases S0–S7).
...
Placa: Freenove ESP32-S3 CAM N16R8 — porta serial COM12 histórica
(MAC `20:6e:f1:b2:3c:f4`); **confirmar por MAC se replugado**. A placa hoje
roda o firmware do NoiseBot v1: não flashear sem decisão explícita do usuário.
...
- Constantes de hardware apenas em `firmware/components/hal/nb_hw_config.h` —
  nunca hardcoded em lógica. GPIOs da câmera (4–18) são fixos: nunca realocar.
